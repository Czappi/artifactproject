...
class NewestMangaListBloc extends MangaListBloc with DiagnosticableTreeMixin {
  ...
  NewestMangaListBloc(this.context);
  ...
  Stream<MangaListState> mapEventToState(MangaListEvent event) async* {
    ...
    if (event is LoadPageEvent) {
      // new items
      var mp =
          await context.read<ManganatoAPI>().newestMangaPage(event.pageKey);
    ...
    if (event is RefreshPageEvent) {
      ...
      var mp = await context
          .read<ManganatoAPI>()
          .newestMangaPage(pagingController.firstPageKey);
